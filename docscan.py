...
def extract_aadhar_details(image_path):
    ...
    for i, line in enumerate(texts):
        if re.search(DOB_PATTERN, line):
            ...
            if not any(bad in context for bad in DOB_BLACKLIST_CONTEXT):
                dob = re.search(DOB_PATTERN, line).group(0)
                if i > 0:
    ...
    for line in texts:
        if "MALE" in line: gender = "MALE"
        if "FEMALE" in line: gender = "FEMALE"
...
def extract_voter_details(image_path):
    ...
    return details
print(extract_voter_details("C:/Users/souri/PycharmProjects/intern/Screenshot 2025-06-04 145211.png"))
def extract_passport_details(image_path: str) -> Dict[str, str]:
    ...
    # Passport Number
    if i < len(filtered_data) and re.fullmatch(passport_no_pattern, filtered_data[i]) or contains_digit(filtered_data[i]):
...
def save_upload_file_tmp(upload_file: UploadFile) -> str:
    tmp_dir = os.path.join(tempfile.gettempdir(), 'tmp_file_storage')
